...
impl AxumServer {
    ...
    pub async fn start(
        ...
        let proxy_routes = Router::new()
            ...
            .route(
                "/v1/responses:stream",
                post(handlers::codex::handle_responses_stream),
            )
            .route(
...
async fn admin_is_auto_launch_enabled(
) -> Result<impl IntoResponse, (StatusCode, Json<ErrorResponse>)> {
    let cfg = config::load_app_config().map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
    ...
    Ok(Json(cfg.auto_launch))
}

#[derive(Deserialize)]
struct AutoLaunchToggleRequest {
    enable: bool,
}

async fn admin_toggle_auto_launch(
    Json(payload): Json<AutoLaunchToggleRequest>,
) -> Result<impl IntoResponse, (StatusCode, Json<ErrorResponse>)> {
    let mut cfg = config::load_app_config().map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse { error: e }),
        )
    })?;
    cfg.auto_launch = payload.enable;
    config::save_app_config(&cfg).map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse { error: e }),
        )
...
mod tests {
    ...
    #[tokio::test]
    async fn admin_toggle_auto_launch_persists_web_config_state() {
        let data_dir = std::env::temp_dir().join(format!(
            "llm-proxy-manager-autostart-test-{}",
            uuid::Uuid::new_v4()
        ));
        let _guard = EnvGuard::set("ABV_DATA_DIR", data_dir.to_string_lossy().to_string());

        let response = admin_toggle_auto_launch(Json(AutoLaunchToggleRequest { enable: true }))
            .await
            .into_response();
