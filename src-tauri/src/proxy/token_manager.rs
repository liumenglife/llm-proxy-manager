...
impl TokenManager {
            ...
            if now >= token.timestamp - 90 {
                ...
                if let Some(latest) = latest_token_opt {
                    if now < latest.timestamp - 90 {
                        ...
                        match token.provider.as_str() {
                            "codex" => {
                                match crate::modules::oauth_codex::refresh_codex_token(
                                    &token.refresh_token,
                                )
                                .await
                                {
                                    ...
                                    Err(e) => {
                                        tracing::error!(
                                            "Codex Token 刷新失败 ({}): {}，尝试下一个账号",
                                            token.email,
                                            e
                                        );
                                        last_error =
                                            Some(format!("Codex token refresh failed: {}", e));
                                        attempted.insert(token.account_id.clone());
            ...
            let project_id = if let Some(pid) = project_id {
                ...
                if let Some(mut entry) = self.tokens.get_mut(&token.account_id) {
                    if let Some(pid) = &entry.project_id {
                        ...
                        match crate::proxy::project_resolver::fetch_project_id(&entry.access_token)
                            .await
                        {
                            Ok(pid) => {
                                entry.project_id = Some(pid.clone());
    ...
    pub async fn get_token_by_email(
        ...
        match provider.as_str() {
            ...
            _ => {
                match crate::modules::oauth::refresh_access_token(&refresh_token, Some(&account_id))
                    .await
                {
                    Ok(token_response) => {
