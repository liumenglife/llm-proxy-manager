...
fn build_codex_model_catalog() -> Vec<ModelDef> {
    vec![
        ...
        ModelDef {
            id: "Codex Max GPT-5.1",
            name: "Codex Max GPT-5.1",
            context_limit: 200_000,
...
mod tests {
    ...
    fn test_sync_preserves_existing_providers() {
        ...
        let result =
            apply_sync_to_config(config, "http://localhost:3000", "test-api-key", None, false);
    ...
    fn test_clear_removes_antigravity_provider() {
        ...
        assert!(
            ...
            "llm-proxy-manager should be removed"
        );
        assert!(
            provider.get("google").is_some(),
    ...
    fn test_clear_legacy_removes_antigravity_models() {
        ...
        assert!(
            ...
            "non-manager model should be preserved"
        );
    }
    ...
    fn test_sync_codex_provider() {
        let config = serde_json::json!({});
        let result =
            apply_sync_to_config(config, "http://localhost:3000", "test-api-key", None, true);
        let provider = result.get("provider").unwrap();
